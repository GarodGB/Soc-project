...
def _mock_ai_answer(prompt: str, system: str = "") -> str:
            ...
            "critical_gaps": [
                {
                    ...
                    "risk_level": "high",
                    "why_important": "Credential attacks are common and need strong telemetry coverage.",
                    "quick_win": True
                },
                {
                    "technique_id": "T1021",
                    ...
                    "risk_level": "high",
                    "why_important": "Attackers use remote services to move between systems.",
                    "quick_win": True
                }
            ],
            ...
            "recommended_next_detections": [
                {
                    "technique_id": "T1110",
                    "technique_name": "Brute Force",
                    "suggested_rule_title": "Multiple Failed Logons Followed by Success",
                    "effort": "low",
                    "impact": "high"
                },
                {
                    "technique_id": "T1021",
                    "technique_name": "Remote Services",
                    "suggested_rule_title": "Suspicious Remote Service Execution",
                    "effort": "medium",
                    "impact": "high"
                }
            ],
    ...
    if '"validation_summary"' in prompt and '"test_cases"' in prompt:
        return json.dumps({
            ...
            "improvement_suggestions": [
                "Add more precise process and parent-process conditions",
    ...
    if '"deployment_steps"' in prompt and '"testing_procedure"' in prompt:
        return json.dumps({
            ...
            "prerequisites": {
                "log_sources": ["Sysmon", "Windows Security Events"],
                "audit_policy": ["Process Creation"],
                "siem_requirements": ["Sigma-compatible rule pipeline"]
            },
            "deployment_steps": [
                {
                    "step": 1,
                    "title": "Review rule logic",
                    "description": "Validate fields and event IDs against available telemetry.",
                    "command": ""
                },
                {
                    "step": 2,
                    "title": "Deploy in test mode",
                    "description": "Run against sample logs before production.",
                    "command": ""
                }
            ],
            "tuning_guide": {
                "initial_threshold": "Start low, then tune after observing alert volume.",
                "allowlist_candidates": ["admin scripts", "backup tools"],
                "environment_specific": "Adjust fields to match your SIEM schema."
            },
            "false_positive_handling": {
                "known_fps": ["legitimate admin activity"],
                "triage_steps": ["check user", "check host", "review parent process"],
                "escalation_criteria": "Escalate if activity is unauthorized or repeated."
            },
            "testing_procedure": {
                "pre_deployment_test": "Run validation cases",
                "validation_command": "Use Test Runner",
                "expected_alert": "One alert for the positive sample"
            },
            "maintenance": {
                "review_frequency": "monthly",
                "update_triggers": ["new false positives", "new ATT&CK procedure"],
    ...
    if 'return only valid json' in lower and '"results"' in prompt:
        return json.dumps({
            "results": [],
            "summary": "Demo Mode: local AI search is enabled, but semantic ranking requires a real API key."
        })
...
def get_client():
    ...
    if anthropic is None:
        raise HTTPException(
            status_code=503,
            detail="anthropic package is not installed. Run: pip install -r requirements.txt"
        )
...
def suggest_rule(req: SuggestRuleRequest):
    ...
    try:
        row = conn.execute(
            "SELECT * FROM mitre_techniques WHERE technique_id = %s",
            (req.technique_id,)
        ).fetchone()
    ...
    if req.save and data.get("primary_technique_id"):
        ...
        try:
            existing = conn.execute("""
                ...
                WHERE detection_id = %s AND technique_id = %s
            ...
            conn.execute(
                "UPDATE detections SET tags = %s WHERE detection_id = %s",
            ...
            conn.execute("""
                INSERT INTO validation_cases
                    (detection_id, detection_title, attack_name,
                     ...
                     status, source)
                VALUES (%s, %s, %s, %s, %s, %s, 'untested', 'ai')
            """, (
                detection_id,
                detection["title"],
...
def _get_platform_context() -> str:
    ...
    try:
        ...
        sev = {
            r[0]: r[1]
            for r in conn.execute("SELECT severity, COUNT(*) FROM detections GROUP BY severity").fetchall()
        }

        plat = {
            r[0]: r[1]
            for r in conn.execute("SELECT platform, COUNT(*) FROM detections GROUP BY platform").fetchall()
        }
        ...
        return (
            ...
            f"- {total_det} detection rules ({json.dumps(sev)})\n"
            f"- {total_tech} MITRE techniques in database\n"
            f"- {covered} techniques covered ({coverage_pct}% coverage)\n"
            f"- {total_telem} telemetry sources\n"
            f"- Platform breakdown: {json.dumps(plat)}\n"
        )
    except Exception:
...
SENSITIVE_PATTERNS = {
    ...
    "email": r'\b[A-Za-z0-9._%+-]+@[A-Z|a-z0-9.-]+\.[A-Z|a-z]{2,}\b',

    "id_value": r'(?im)\b(?:id|user_id|userid|username|user|account|account_name|name)\s*[:=]\s*["\']?([A-Za-z][A-Za-z0-9._\\-]{1,})["\']?',
    "number_value": r'(?im)\b(?:number|phone|mobile|employee_id|employeeid|customer_id|customerid|ticket_id|ticketid|case_id|caseid)\s*[:=]\s*["\']?([+]?\d[\d ._-]{1,})["\']?',
    ...
    "json_user": r'"(?:email|username|userPrincipalName|actor_user_name|user|name)":\s*"(.*?)"',
    "json_ip": r'"(?:ipAddress|ipaddr|IpAddress|source_ip|destination_ip)":\s*"(.*?)"',
    "json_host": r'"(?:hostname|host|computer|city|country|countryOrRegion)":\s*"(.*?)"',
...
def _is_already_masked(value: str) -> bool:
    ...
    return value.startswith(masked_prefixes)
    ...
    for field_type, pattern in SENSITIVE_PATTERNS.items():
        ...
        for match in matches:
            if match.lastindex and match.lastindex >= 1:
                ...
                if _is_already_masked(original_value):
                    continue
                ...
                masked_fields.append({
                    "type": field_type,
                    "original_preview": original_value[:3] + "***",
                    "masked_as": masked_value
                })

            else:
                original = match.group(0)

                if _is_already_masked(original):
                    continue
                ...
                masked_fields.append({
                    "type": field_type,
                    "original_preview": original[:3] + "***",
                    "masked_as": masked_value
                })

    mask_id = hashlib.sha256(
        f"{req.log_text}{datetime.utcnow().isoformat()}".encode()
    ).hexdigest()[:16]

    _mask_store[mask_id] = {
        "mapping": mask_mapping,
        "created_at": datetime.utcnow().isoformat(),
        "original_text": req.log_text
    }

    return MaskResponse(
        masked_text=log_text,
...
def unmask_log_data(req: UnmaskRequest):
    ...
    if req.role not in allowed_roles:
        raise HTTPException(
            status_code=403,
            detail=f"De-masking requires Admin or Engineer role. Your role: {req.role}"
        )
...
{{
  ...
  "summary": "Found X matching rules..."
}}"""
...
Score on:
- Logic Accuracy (X/20)
- Coverage Breadth (X/20)
- False Positive Management (X/20)
- MITRE Alignment (X/20)
- Operational Readiness (X/20)

Give TOP 3 IMPROVEMENTS and VERDICT:
Production Ready / Needs Improvement / Not Ready."""
...
def generate_hunt_plan(req: HuntRequest):
    ...
    try:
        ...
        covered = conn.execute(
            "SELECT COUNT(DISTINCT technique_id) FROM detection_technique_mapping"
        ).fetchone()[0]
    finally:
        conn.close()
...
- {active} active
- {total_tech} techniques
- {covered} covered ({coverage_pct}%)
...
def ai_health():
    ...
    return {
        ...
        # Never echo any part of a secret — the caller only needs to know
