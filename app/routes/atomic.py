...
@router.post("/runs", status_code=201, dependencies=[Depends(require_write_access)])
def create_atomic_run(body: AtomicRunCreate):
    conn = get_connection()
    try:
        _ensure_atomic_runs(conn)
        detection = conn.execute(
            "SELECT title FROM detections WHERE detection_id = %s", (body.detection_id,)
        ).fetchone()
        if not detection:
            raise HTTPException(status_code=404, detail="Detection not found")
...
@router.patch("/runs/{run_id}", dependencies=[Depends(require_write_access)])
def update_atomic_run(run_id: int, body: AtomicRunUpdate):
    allowed = {"planned", "running", "completed", "failed", "cancelled"}
    conn = get_connection()
    try:
        _ensure_atomic_runs(conn)
        row = conn.execute("SELECT * FROM atomic_runs WHERE run_id = %s", (run_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Atomic run not found")
        ...
        conn.execute(
            f"UPDATE atomic_runs SET {set_clause} WHERE run_id=%s",
...
@router.post("/runs/{run_id}/sample-log", status_code=201, dependencies=[Depends(require_write_access)])
def attach_atomic_log(run_id: int, body: AtomicLogIngest):
    ...
    try:
        ...
        cur = conn.execute("""
            ...
            VALUES (%s, %s, %s, NULL, 'untested', NULL, %s, %s, %s)
            RETURNING case_id
        """, (
            run["detection_id"],
            body.sample_event,
