...
def get_detections(
    ...
    try:
        ...
        if search:
            sql += " AND (title ILIKE %s OR tags ILIKE %s OR author ILIKE %s)"
            like = f"%{search}%"
    ...
    try:
        ...
        conn.execute("""
            UPDATE detections SET
              title=%s, description=%s, severity=%s, status=%s, platform=%s,
              author=%s, falsepositives=%s, raw_yaml=%s, tags=%s,
              updated_at=now()::text
...
def delete_detection(detection_id: str):
    ...
    try:
        ...
        conn.execute("DELETE FROM detection_technique_mapping WHERE detection_id = %s", (detection_id,))
        conn.execute("DELETE FROM detection_telemetry WHERE detection_id = %s", (detection_id,))
