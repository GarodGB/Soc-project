...
def get_validation_cases(
    ...
    try:
        ...
        if detection_id:
            sql += " AND detection_id = %s"
            params.append(detection_id)
        if status:
            sql += " AND status = %s"
            params.append(status)
        sql += " ORDER BY case_id DESC LIMIT %s"
        params.append(limit)
...
def get_matching_rules_for_case(case_id: int, limit: int = Query(20, le=100)):
    ...
    try:
        case = conn.execute(
            "SELECT * FROM validation_cases WHERE case_id = %s", (case_id,)
        ).fetchone()
        if not case:
...
@router.patch("/{case_id}", dependencies=[Depends(require_write_access)])
    ...
    try:
        ...
        if "detection_id" in updates:
            detection = conn.execute(
                "SELECT title FROM detections WHERE detection_id = %s", (updates["detection_id"],)
            ).fetchone()
            if not detection:
        ...
        conn.execute(
            f"UPDATE validation_cases SET {set_clause} WHERE case_id=%s",
...
def simulate_detection_against_case(req: SimulationRequest):
    ...
    try:
        _ensure_simulation_results(conn)
        detection = conn.execute(
            "SELECT * FROM detections WHERE detection_id = %s", (req.detection_id,)
        ).fetchone()
        if not detection:
            raise HTTPException(status_code=404, detail="Detection not found")

        case = conn.execute(
            "SELECT * FROM validation_cases WHERE case_id = %s", (req.case_id,)
        ).fetchone()
        if not case:
            raise HTTPException(status_code=404, detail="Validation case not found")
        ...
        conn.execute("""
            ...
            SET actual_result=%s, status=%s, tested_at=%s
            WHERE case_id=%s
...
@router.post("/{case_id}/run", dependencies=[Depends(require_write_access)])
...
@router.post("/test-rule/{detection_id}", dependencies=[Depends(require_write_access)])
def test_rule(detection_id: int):
    ...
    try:
        _ensure_simulation_results(conn)
        detection = conn.execute(
            "SELECT * FROM detections WHERE detection_id = %s", (detection_id,)
        ).fetchone()
        if not detection:
            raise HTTPException(status_code=404, detail="Detection not found")
        detection = dict(detection)
        ...
        cases = conn.execute(
            "SELECT * FROM validation_cases WHERE detection_id = %s",
            (detection_id,),
        ).fetchall()
...
def run_all_cases():
    ...
    try:
        ...
        for c in cases:
            c = dict(c)
            detection = conn.execute(
                "SELECT * FROM detections WHERE detection_id = %s", (c["detection_id"],)
            ).fetchone()
            if not detection:
                simulated = "no_fire"
                passed = False
