...
def main():
    ...
    if "details" not in cols:
        conn.execute("ALTER TABLE telemetry_sources ADD COLUMN details TEXT")
    ...
    for name, detail_data in DETAILS.items():
        row = conn.execute(
            "SELECT source_id FROM telemetry_sources WHERE name = %s", (name,)
        ).fetchone()
        if row:
            detail_json = json.dumps(detail_data, ensure_ascii=False)
            conn.execute("UPDATE telemetry_sources SET details = %s WHERE source_id = %s",
                      (detail_json, row[0]))
